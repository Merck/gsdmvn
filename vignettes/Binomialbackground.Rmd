...
```{r, message=FALSE}
p_c <- tibble::tibble(Stratum = c("S1", "S2", "S3"), Rate = c(.3, .37, .6))
p_e <- tibble::tibble(Stratum = c("S1", "S2", "S3"), Rate = c(.25, .3, .5))
ratio <- 1
ratio_strata_c <- tibble::tibble(Stratum = c("S1", "S2", "S3"), ratio = c(4, 5, 6))
...
```{r}
run_simulation <- function(N, weight_method){
  
  ans <- NULL
  
  N_S1 <- ceiling(N * 4 / sum(4:6)) * (1:3) / 3
  N_S2 <- ceiling(N * 5 / sum(4:6)) * (1:3) / 3
  N_S3 <- ceiling(N * 6 / sum(4:6)) * (1:3) / 3
  ...
  for (simu in 1:1e3) {
    ...
    res <- switch (weight_method,
      "invar_H0" = {
        gs_power_npe(theta = mean(x$rd_invar_H0), 
                     info = x$info_invar_H0, 
                     info_scale = 2,
                     binding = FALSE,
                     upper = gs_b, 
      ...
      "invar_H1" = {
        gs_power_npe(theta = mean(x$rd_invar_H1), 
                     info = x$info_invar_H1, 
                     info_scale = 2,
                     binding = FALSE,
                     upper = gs_b, 
      ...
      "ss" = {
        gs_power_npe(theta = mean(x$rd_ss), 
                     info = x$info_ss, 
                     info_scale = 2,
                     binding = FALSE,
                     upper = gs_b, 
...
# run simulations
ans <- do.call(cbind, 
               lapply(list(list("ss", 1095), list("invar_H0", 1131), list("invar_H1", 1130)), 
                      function(x){return(run_simulation(N = x[[2]], weight_method = x[[1]]))})
               ) 

ans <- tibble::tibble(simu_ID = ans$simu_ID,
                      ss = ans[, 3],
                      invar_H0 = ans[, 6],
                      invar_H1 = ans[, 9])

save(ans, file = "./vignettes/fixture/rd_simu_power_3_weight.Rdata")
```
...
ans %>% 
  summarise(ss = mean(ss), invar_H0 = mean(invar_H0), invar_H1 = mean(invar_H1)) %>% 
  gt::gt() %>% 
