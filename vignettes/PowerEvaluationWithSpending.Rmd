...
+-----------------+-------------------------+---------------------------------------+---------------------------+
| Parameter       | Hypothesized $\theta$  | Test                                   | $\widehat{\hbox{Var}}(X)$ |
+=================+:======================:+:======================================:+:======:+
| Risk difference | $\theta =p_1 - p_2$    | $X_{rd}=\hat p_1-\hat p_2-\theta$      | $\hat V_{rd}=\frac{1+r}{N}\left(\tilde p_{1,rd}\tilde q_{1,rd}+\tilde p_{2,rd} \tilde q_{2,rd}/r \right)$ |
+-----------------+:-----------------------------------------+----------------------+--------+
| Risk ratio      | $\theta=p_2/p_1$       | $X_{rr}=\theta_{rr}\hat p_1 -\hat p_2$ | $\hat V_{rr}=\frac{1+r}{N}\left(\theta_{rr}^2\tilde p_{1,rr}\tilde q_{1,rr}+\tilde p_{2,rr} \tilde q_{2,rr}/r\right)$  |
+-----------------+-----------------------------------------+-----------------------+--------+


FOLLOWING 2 EQUATIONS MAY BE REMOVED; THEY ARE HERE TO ENSURE TABLE EQUATIONS ARE NOT LOST. 

$\hat V_{rd}=\frac{1+r}{N}\left( \tilde p_{1,rd}\tilde q_{1,rd}+\tilde p_{2,rd} \tilde q_{2,rd}/r \right)$

$\hat V_{rr}=\frac{1+r}{N}\left(\theta_{rr}^2\tilde p_{1,rr}\tilde q_{1,rr}+\tilde p_{2,rr} \tilde q_{2,rr}/r\right) /p_1$                                       
...
```{r echo=TRUE}
info_rd(theta=c(0,.1,.05)) %>% gt() %>% 
  fmt_number(decimals=0, columns = c("N", "N1", "N2", "ratio")) %>%
  fmt_number(decimals=3, columns = c("p1", "p2", "p1_tilde", "p2_tilde", "theta")) %>%
  fmt_number(decimals=1, columns = c("info", "infot", "info"))
```
...
info_rr <- function(N=200, ratio=1, p1 = .2, p2 = .1, theta = 0){
    ...
    N1 <- N / (1 +ratio)
    N2 <- ratio * N / (1 + ratio)
    RR <- exp(theta) # Assumed risk ratio
    a <- (1 + ratio)
...
```{r}
gsDesign::nBinomial(n=200, p1 = .2, p2 = .1, delta0 = log(c(1, .5, .75)), alpha=.025, beta = .1, 
                    scale = "RR", outtype = 3) %>% 
     dplyr::transmute(N = n, p1 = p1, p2 = p2, p1t = p10, p2t = p20, theta = delta0, 
