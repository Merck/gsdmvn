...
library(gsDesign)
library(dplyr)
context("Developer Tests of gs_power_npe()")
...
test_that("Basic call to gs_power_npe fails", {
  ...
  x <- try(
    gs_power_npe(
      ...
      info = 1:3, info0 = NULL, 
      binding = FALSE,
      upper = gs_b, upar = 4:2, test_upper = TRUE,
      lower=gs_b, lpar= -1:1, test_lower = TRUE)
    )
...
#   ) %>% filter(Bound == "Upper")
  ...
  y1s <- gsProbability(
    k = 3, 
    theta = .1, n.I = (1:3) * 400, a = rep(-20, 3),
    b = gsDesign(k = 3, test.type = 1, sfu = sfLDOF)$upper$bound)
  expect_equal((x1s %>% filter(hypothesis == "H1"))$Z, 
               y1s$upper$bound, 
               tol = 1e-4)
  expect_equal((x1s %>% filter(hypothesis == "H1"))$Probability, 
               cumsum(y1s$upper$prob), 
               tol = 1e-5)
...
test_that("Comparison of gs_power_npe to gsDesign fails", {
  ...
  expect_lt(max(abs(b1$Probability - (b1a  %>% filter(hypothesis == "H1"))$Probability)), 2e-6)
})
