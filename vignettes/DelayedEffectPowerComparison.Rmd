...
failRates <- tibble(Stratum = "All",
                            ...
                            hr = c(1, .6),
                            dropoutRate = 0.001)
enrollRates %>% gt() %>% tab_header(title = "Enrollment Table")
failRates %>% gt() %>% tab_header(title = "Failure Table")
```
    ...
    rbind(tab, 
        cbind(
              FH05 %>% 
                summary_bound() %>% 
                ungroup() %>% 
                transmute("Study duration" = FH05$analysis$Time[1], 
                          N = FH05$analysis$N[1], 
                          Events = FH05$analysi$Events[1], 
                          "Events/N" = Events/N, 
                          AHR = as.numeric(FH00[4]), 
                          "FH(0, 0.5) power" = `Alternate hypothesis`),
              FH00 %>% 
                ungroup() %>% 
                transmute("Logrank power" = `Alternate hypothesis`, 
