...
$$Z_k(\theta)=\frac{\hat p_{1k}-\hat p_{2k} - \theta}{V(\hat p_{1k}, \hat p_{2k}, N_{1k}, N_{2k},\theta)^{1/2}}.$$
...
### Computing $\tilde p_{1,rd}, \tilde p_{2,rd}, \bar p_{1,rd}, \bar p_{2,rd}$
...
asymptotic approximations can be compute with `info_rd` function defined
below. The values $\tilde p_1$, $\tilde p_2$ from above are computed
here.
...
```{r}
power_rd <- function(N=200, ratio=1, p1 = .2, p2 = .1, theta = 0,
                    binding = FALSE,
                    upper = gs_b,
                    ...
                    lpar = c(qnorm(.1), rep(-Inf, length(N) - 1)),                                        test_upper = TRUE,
                    test_lower = TRUE,
                    r = 18,
                    tol = 1e-6
){
  x <- info_rd(N=N, ratio=ratio, p1 = p1, p2 = p2, theta = theta)
