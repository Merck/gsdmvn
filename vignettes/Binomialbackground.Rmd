...
The example discussed in this section is an unstratified fixed design with eual sized qgoups to detect a 30\% reduction in mortality associated with congestive heart failre, where the 1-year mortality in the control group is assumed to be no greater than 0.4.
So $p_C=0.4, p_E = .28$. 
Under the null hypothesisi, we assume $p_C=p_E =0.34$.
...
The above logic is implemented in `gs_info_rd`.
```{r}
gs_info_rd(
...
```{r}
x <- gs_design_npe(theta = abs(p_e - p_c), 
              info = info_H1, 
              info0 = info_H0, 
              info_scale = 2,
              alpha = .025, 
              ...
              lpar = list(par = -Inf))
cat("The sample size calculated by `gsdmvn` is ", x$info[1] / info_H1 * N) 
```
...
### EAST - Unpooled
```{r label = esatunpool, echo = FALSE, fig.cap = "Sample size calculated by EAST", out.width = '90%'}
knitr::include_graphics("./figures/east_n.png")
```
