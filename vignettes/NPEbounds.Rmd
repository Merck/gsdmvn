...
We consider different boundary types as in the **gsDesign** package.
We include the `test.type` argument from the `gsDesign::gsDesign()` function for reference. 
  ...
  ------------- ------------------- -------------------    ------------------------------------------
        ...
        5           $\alpha(0)$        $\beta_i(\theta_1)$ $\theta$-spending with binding futility   
        6         $\alpha^{+}(0)$      $\beta_i(\theta_1)$ $\theta$-spending with non-binding futility

  : Boundary crossing probabilities used to set boundaries
