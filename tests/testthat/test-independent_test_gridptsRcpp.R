...
testthat::test_that("compare gridptsRcpp results with gsDesign::normalGrid results", {
  test1 <- gsdmvn::gridptsRcpp(r = 18, mu = 4, a = -Inf, b = Inf)
  x <- gsDesign::normalGrid(r = 18, bounds = c(-40, 40), mu = 4, sigma = 1)
  expect_equal(test1$w, x$gridwgts)
  expect_equal(test1$z, x$z)

  test2 <- gsdmvn::gridptsRcpp(r = 18, mu = 2, a = -Inf, b = Inf)
