...
# User defined bound
gs_power_combo_test1 <- gs_power_combo(enrollRates,
                                       failRates,
                                       fh_test,
                                       upar = c(3, 2, 1),
                                       lpar = c(-1, 0, 1))
test_that ( "calculate analysis number as planed",{
  expect_equal(max(fh_test$Analysis), max(gs_power_combo_test1$Analysis ))
})
test_that ( "calculate analysisTimes as planed",{
  expect_equal(unique(fh_test$analysisTimes), unique(gs_power_combo_test1$Time))
})
...
for (i in 1:max(fh_test$Analysis)) {
  test_that ( "calculate N and each analysis Events N as planed",{
    ...
    expect_equal(event, unique(gs_power_combo_test1$Events)[i], tolerance = 0.01)
  })
}


# Minimal Information Fraction derived bound
gs_power_combo_test2 <- gs_power_combo(enrollRates, failRates, fh_test,
                                       upper = gs_spending_combo,
                                       upar  = list(sf = gsDesign::sfLDOF, total_spend = 0.025),
                                       lower = gs_spending_combo,
                                       lpar  = list(sf = gsDesign::sfLDOF, total_spend = 0.2))
test_that ( "calculate analysis number as planed",{
  expect_equal(max(fh_test$Analysis), max(gs_power_combo_test2$Analysis ))
})
test_that ( "calculate analysisTimes as planed",{
  expect_equal(unique(fh_test$analysisTimes), unique(gs_power_combo_test2$Time))
})
test_tEvents<-function(enrollRates = tibble::tibble(Stratum = "All",
                                                    duration = c(2, 2, 10),
                                                    rate = c(3, 6, 9) * 5),
...
for (i in 1:max(fh_test$Analysis)) {
  test_that ( "calculate N and each analysis Events N as planed",{
    event <- test_tEvents(enrollRates = enrollRates,
                          failRates = failRates,
                          td = unique(fh_test$analysisTimes)[i])
    expect_equal(event, unique(gs_power_combo_test2$Events)[i], tolerance = 0.01)
  })
}
