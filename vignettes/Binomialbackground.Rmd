...
```{r,eval=FALSE}
y <- gs_design_npe(theta = 0.43,#x$theta - x$theta0, 
...
The above logic is implement in `gs_design_rd()` to calcute the sample size given fixed power in one-step.
...
## Unstratified Group Seqential Design  {.tabset}
...
We desire 90\% power for a two-sided test for two propotions at $\alpha = 0.05$.
And we would like to calculate the sample size to achieve the 90\% power.
...
## Stratified Group Seqential Design  {.tabset}
