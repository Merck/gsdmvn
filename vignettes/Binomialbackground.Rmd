...
- $K$: total number of analysis (including the final analysis) in the group sequential design. If it is the fixed design, then $K= 1$.

- $S$: total number of stratum. If it is un-stratified population, then $S=1$.

- $w_{s,k}$: the weight assigned for the $s$-th strata at the $k$-th analysis. 
  + If it is un-stratified population, then $S = 1$ and $w_{s,k} = 1$ for any $s=1,\ldots, S$ and $k=1,\ldots, K$.
  + If it is stratified population, see Section 4
...
- $N_k$: total sample size at the $k$-th analysis, i.e., 
$$
  N_k = \sum_{s=1}^S N_{k,s} = \sum_{s=1}^S (N_{C,k,s} + N_{E,k,s}).
$$

- $N$: total sample size, i.e., 
$$
  N = \sum_{k=1}^K N_k = \sum_{k=1}^K \sum_{s=1}^S (N_{C,k,s} + N_{E,k,s}).
$$
...
- $d_{k,s}$: an indicator whether is an outcome is bad or good, i.e.,
$$
  d_{k,s}
  = 
  \left\{
...
- $\theta_{k,s}$: the natural parameter risk difference at the $k$-th analysis of the $s$-th strata is denoted by 
$$
...
$$
  ...
  + Under the alternative hypothesis, we have
$$
  \theta_{H_1,k} = \frac{\sum_{s=1}^S w_{k,s} (p_C - p_E)}{\sqrt{\sum_{s=1}^S w_{k,s}^2 \sigma_{H_1, k,s}^2}},
$$
which can be estimated as
$$
  \widehat\theta_{H_1,k} = \frac{\sum_{s=1}^S w_{k,s} (\widehat p_C - \widehat p_E)}{\sqrt{\sum_{s=1}^S w_{k,s}^2 \widehat\sigma_{H_1, k,s}^2}}.
$$
