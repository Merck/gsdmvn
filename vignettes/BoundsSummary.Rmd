...
Then users can use `gsdmvn::gt_format()` to format the above R table into a gt table. 
...
The above table can also be format by `gt_format()` as
```{r}
summary_bound(x_wlr, method = "WLR") %>% 
...
```{r}
x_combo$bounds %>% gt::gt()
```


```{r}
x_combo$analysis %>% gt::gt()
```
