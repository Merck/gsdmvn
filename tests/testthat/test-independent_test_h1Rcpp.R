test_that("h1Rcpp() returns results as expected ",{
  #the design
  gstry <- gsDesign::gsDesign(k = 3,
                              sfl = gsDesign::sfLDOF,
                              delta = 0)
  #probabilities calculated based on function h1()
  upper.null <- sum(h1Rcpp(theta = gstry$theta[1],
                   I = gstry$n.I[1],
                   a = gstry$upper$bound[1],
                   b = Inf)$h)
  upper.alt <- sum(h1Rcpp(theta = gstry$theta[2],
                  I = gstry$n.I[1],
                  a = gstry$upper$bound[1],
                  b = Inf)$h)
  lower.null <- sum(h1Rcpp(theta = gstry$theta[1],
                   I = gstry$n.I[1],
                   a = -Inf,
                   b = gstry$lower$bound[1])$h)
  lower.alt <- sum(h1Rcpp(theta = gstry$theta[2],
                  I = gstry$n.I[1],
                  a = -Inf,
                  b = gstry$lower$bound[1])$h)
...
test_that("h1Rcpp() returns probability almost zero for extreme case",{
  exmtest1 <- sum(h1Rcpp(theta = 9, I = 0.5, a = -Inf, b=0)$h)
  exmtest2 <- sum(h1Rcpp(theta = 1, I = 0.5, a = 9, b = Inf)$h)
