...
cat("The sample size calculated by `gsdmvn` is ", y$info[1] / x$info[1]) 
```
...
```{r}
tibble::tibble(gsdmvn = x_gsdmvn$analysis$N[3],
               gsDesign = x_gsDesign$n.I,
               EAST_unpooled = c(NA, NA, 1850),
               EAST_pooled = c(NA, NA, 1857)) %>% gt::gt()
```
...
gs_design_rd(
  ...
  beta = .1,                    
  ratio = 1,
  ratio_stratum = tibble::tibble(Stratum = c("S1", "S2", "S3"), Ratio = 1:3),
  weight = "ss",
  upper = gs_b,
  lower = gs_b,
