...
summary_bound(positive) %>% 
  ...
  mutate(# calculate the spending time, which is info0 / max(info0)
...
gs_power_ahr(
  ...
  events = NULL,
  analysisTimes = (positive_60_power$analysis %>% filter(hypothesis == "H1"))$Time %>% unique(),
...
gs_power_ahr(
  ...
  failRates = failRates,
  events = 1:2,
  analysisTimes = (positive_40_power$analysis %>% filter(hypothesis == "H1"))$Time %>% unique(),
  upper = gs_spending_bound,
  upar = upar_overall_planned_IF,
  lower = gs_b,
  lpar = rep(-Inf,2)) %>% 
