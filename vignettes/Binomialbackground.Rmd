...
- $d$: an indicator whether is an outcome is failure (bad outcome) or response (good outcome), i.e.,
$$
  d
  = 
  \left\{
  ...
  \right.
$$
Here we assume if $\exists s^* \in \{1, \ldots, S\}$, s.t., $p_{C,s^*} < p_{E,s^*}$, then $p_{C,s} < p_{E,s}, \forall s \in \{1, \ldots, S\}$, and vice versa.
...
- $\widehat p_{C,k,s}, \widehat p_{E,k,s}$: observed rates of the control/treatment group at the $k$-th analysis of the $s$-th strata, i.e., 
$$
...
- $\delta_{s}$: the **planned** risk difference under $H_1$ at any $k$-th analysis of the $s$-th strata is denoted by
$$
...
$$
We have $E(\widehat\theta_{k,s}) = \theta_{s}, \;\forall k = 1, \ldots, K$.
...
$$
where $\widehat\sigma^2_{k,s} = \widehat{\hbox{Var}}(\widehat p_C -\widehat p_E)$. 
And the value of  $\widehat\sigma^2_{k,s}$ depends on the hypothesis and design, i.e., whether it is a superiority design, or non-inferiority design, or super-superiority design. 
We will discuss $\widehat\sigma^2_{k,s}$ in the following 3 subsections.


## Superiority Design

A superiority design ($\delta_{s}^{null} = 0$) is to show that experimental group is superior to the control group above some thresholds.
Its hypothesis is
$$
...
$$
  ...
  \end{array}
$$
where $p_{k,s}^{pool} = (p_{C,s} N_{C,k,s}  + p_{E,s} N_{E,k,s}) / (N_{C,k,s} + N_{E,k,s})$ and $\widehat p_{k,s}^{pool} = (x_{C,k,s} + x_{E,k,s}) / (\widehat N_{C,k,s} + \widehat N_{E,k,s}).$
...
The example discussed in this section is an unstratified fixed design with equal sized groups to detect a 30\% reduction in mortality associated with congestive heart failure, where the 1-year mortality in the control group is assumed to be no greater than 0.4.
So $p_C=0.4, p_E = .28$. 
Under the null hypothesis, we assume $p_C=p_E =0.34$.
We desire 90\% power for a two-sided test for two proportions at $\alpha = 0.05$.
...
Then we calculate the variance under $H_0$ and $H_1$.
Their mathmatical formulation are shown as follows. 
$$
  \begin{array}{ll}
  \sigma^2_{H_0} 
  =
...
Next, we calculate the standarized treatment effect under $H_0$ and $H_1$, whose mathmatical formulation are
$$
  \begin{array}{ll}
  \theta_{H_0} = 0; \\
...
```{r}
y <- gs_design_npe(theta = abs(p_e - p_c), 
                   info = x$info, 
                   info0 = x$info0, 
                   info_scale = 2,
                   alpha = .025, 
...
cat("The sample size calculated by `gsdmvn` is ", y$info[1] / x$info[1] * N) 
```
...
cat("The sample size calculated by `gs_design_rd()` is ", z$analysis$N, "\n")
...
Under the null hypothesis, we assume $p_C=p_E =0.125$.
We desire 90\% power for a two-sided test for two proportions at $\alpha = 0.05$.
And we would like to calculate the sample size to achieve the 90\% power.

### `gsdmvn`

To calculate the sample size, one can use `gs_design_rd()`.
The logic of `gs_design_rd()` is to calculate the sample size of fixed design first.
```{r, eval=FALSE}
x_fix <- gs_info_rd(
...
y_fix <- gs_design_npe(
    ...
    upper = gs_b,
    lower = gs_b,
    upar = list(par = gsDesign(k = 1, test.type = 1, sfu = sfLDOF, sfupar = NULL)$upper$bound),
    lpar = list(par = -Inf),
    test_lower = FALSE)

cat("The sample size of the fixed design is ", y_fix$info/x_fix$info, ".\n")
```

Then the sample size of the group sequential design is a product of 1834.641 and an inflation factor. 
...
cat("The sample size of the group sequential design is ", y_gs$info/x_gs$info[3], "\n")
```
...
cat("The sample size calculated by `gsdmvn` is ", x$analysis$N)
```
...
```{r, message=FALSE}
p_c <- tibble::tibble(Stratum = c("S1", "S2", "S3"), Rate = c(.4, .5, .6))
p_e <- tibble::tibble(Stratum = c("S1", "S2", "S3"), Rate = c(.25, .3, .5))
ratio <- 1
ratio_strata_c <- tibble::tibble(Stratum = c("S1", "S2", "S3"), ratio = c(1, 2, 3))
...
x <- p_c %>% 
  ...
  mutate(xi_c = round((ratio_strata_c %>% mutate(prop = ratio / sum(ratio)))$prop, 4)) %>% 
  mutate(xi_e = round((ratio_strata_e %>% mutate(prop = ratio / sum(ratio)))$prop, 4)) %>% 
...
Third, we calculate the weighted risk difference and weighted statistical information.
$$
  \left\{
  \begin{array}{ll}
    \delta_{H_0,k} 
...
x <- x %>% 
  ...
  summarise(rd_invar_H0 = sum(weight_invar_H0 * abs(p_c - p_e)),
            ...
            info_ss = 1 / sum(weight_invar_H1^2*p_c*(1-p_c)/N_c + weight_invar_H1^2*p_e*(1-p_e)/N_e),
...
x %>% 
  gt::gt() %>% 
  fmt_number(c(2:4, 6:11), decimals = 4) %>% 
...
tibble::tibble(Method = c("INVAR-H0", "INVAR-H1", "SS"),
               `Sample size` = c(y_invar_H0$info[3] / x$info_invar_H0[3], 
                                 y_invar_H1$info[3] / x$info_invar_H1[3], 
                                 y_ss$info[3] / x$info_ss[3])) %>% 
  gt::gt() %>% 
...
gs_design_rd(
  ...
  lpar = list(par = c(qnorm(.1), rep(-Inf, 2))),
  info_scale = 2,
  binding = FALSE)
```
