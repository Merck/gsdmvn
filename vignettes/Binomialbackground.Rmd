...
tibble(`info_scale = 0` = y_0$info0[1] / x$info0[1],
  ...
  gt::gt() %>% 
  gt::tab_header(title = "The sample size calculated by `gsdmvn` under 3 info_scale")
```
...
```{r, echo=FALSE}
x_gsDesign <- nBinomial(p1 = .28, p2 = .4, delta0 = 0, alpha = .025, sided = 1, beta = .1, outtype = 3)
```
...
### Summary
```{r}
tibble::tibble(gsdmvn_info_scale_0 = z_info_scale_0$analysis$N,
               gsdmvn_info_scale_1 = z_info_scale_1$analysis$N,
               gsdmvn_info_scale_2 = z_info_scale_2$analysis$N,
               gsDesign = x_gsDesign$n, 
               EAST_unpool = 645, 
               EAST_pool = 651) %>% 
  gt::gt() %>% 
  gt::tab_spanner(label = "gsdmvn",
                  columns = c(gsdmvn_info_scale_0, gsdmvn_info_scale_1, gsdmvn_info_scale_2)) %>% 
  gt::tab_spanner(label = "EAST",
                  columns = c(EAST_unpool, EAST_pool)) %>% 
  cols_label(gsdmvn_info_scale_0 = "info_scale = 0",
             gsdmvn_info_scale_1 = "info_scale = 1",
             gsdmvn_info_scale_2 = "info_scale = 2",
...
tibble(`info_scale = 0` = y_gs0$info0 / x_gs$info0[3],
  ...
  gt::gt() %>% 
  gt::tab_header(title = "The sample size calculated by `gsdmvn` under 3 info_scale", subtitle = "under group sequential design")
...
```{r}
x_gsdmvn_info_scale_0 <- gs_design_rd(
  p_c = tibble::tibble(Stratum = "All", Rate = .15),
  p_e = tibble::tibble(Stratum = "All", Rate = .1),
  k = 3,
  IF = 1:3/3,
...
x_gsdmvn_info_scale_1 <- gs_design_rd(
  p_c = tibble::tibble(Stratum = "All", Rate = .15),
  p_e = tibble::tibble(Stratum = "All", Rate = .1),
  k = 3,
  IF = 1:3/3,
...
x_gsdmvn_info_scale_2 <- gs_design_rd(
  p_c = tibble::tibble(Stratum = "All", Rate = .15),
  p_e = tibble::tibble(Stratum = "All", Rate = .1),
  k = 3,
  IF = 1:3/3,
...
# x_gsdmvn_info_scale_1 %>% summary() %>% as_gt()
```
...
```{r}
tibble::tibble(gsdmvn_info_scale_0 = x_gsdmvn_info_scale_0$analysis$N,
               gsdmvn_info_scale_1 = x_gsdmvn_info_scale_1$analysis$N,
               gsdmvn_info_scale_2 = x_gsdmvn_info_scale_2$analysis$N,
...
tibble::tibble(INVAR = x_invar$analysis$N[1:3],
               SS = x_ss$analysis$N[1:3]) %>% 
  gt::gt() %>% 
  gt::tab_header(title = "Sample size calculated by INVAR and SS")
```
...
```{r}
run_simulation <- function(N, weight_method, n_sim, prevalence_ratio, IF, under_H0 = TRUE){
  ...
  for (simu in 1:n_sim) {
    ...
    x_e_S1 <- lapply(diff(c(0, N_S1/2)), function(x){rbinom(n = x, size = 1, prob = p_e$Rate[1])})
    x_e_S2 <- lapply(diff(c(0, N_S2/2)), function(x){rbinom(n = x, size = 1, prob = p_e$Rate[2])})
    x_e_S3 <- lapply(diff(c(0, N_S3/2)), function(x){rbinom(n = x, size = 1, prob = p_e$Rate[3])})
    x_c_S1 <- lapply(diff(c(0, N_S1/2)), function(x){rbinom(n = x, size = 1, prob = p_c$Rate[1])})
    x_c_S2 <- lapply(diff(c(0, N_S2/2)), function(x){rbinom(n = x, size = 1, prob = p_c$Rate[2])})
    x_c_S3 <- lapply(diff(c(0, N_S3/2)), function(x){rbinom(n = x, size = 1, prob = p_c$Rate[3])})
    ...
    ans_new <- tibble::tibble(simu_ID = simu,
...
ans_H0 <- do.call(cbind, 
                  lapply(list(
                    list("ss", ans_math %>% filter(`Calculated under` == "H0", `Weighting method` == "Sample Size") %>% select(`Sample size`) %>% as.numeric()), 
                    list("invar_H0", ans_math %>% filter(`Calculated under` == "H0", `Weighting method` == "INVAR-H0") %>% select(`Sample size`) %>% as.numeric()), 
                    list("invar_H1", ans_math %>% filter(`Calculated under` == "H0", `Weighting method` == "INVAR-H1") %>% select(`Sample size`) %>% as.numeric())), 
...
ans_H1 <- do.call(cbind, 
                  lapply(list(
                    list("ss", ans_math %>% filter(`Calculated under` == "H1", `Weighting method` == "Sample Size") %>% select(`Sample size`) %>% as.numeric()), 
                    list("invar_H0", ans_math %>% filter(`Calculated under` == "H1", `Weighting method` == "INVAR-H0") %>% select(`Sample size`) %>% as.numeric()), 
                    list("invar_H1", ans_math %>% filter(`Calculated under` == "H1", `Weighting method` == "INVAR-H1") %>% select(`Sample size`) %>% as.numeric())), 
                         function(x){return(run_simulation(N = x[[2]], 
                                                           weight_method = x[[1]], 
                                                           n_sim = n_sim, prevalence_ratio = prevalence_ratio, 
                                                           IF = IF, under_H0 = TRUE))})) 
...
ans <- tibble::tibble(simu_ID = rep(ans_H0$simu_ID, 2),
                      ...
                      invar_H0 = c(ans_H0[, 6], ans_H1[, 6]),
                      invar_H1 = c(ans_H0[, 9], ans_H1[, 9]),
                      `calculated under` = c(rep("H0", n_sim), rep("H1", n_sim))) 

save(ans, file = "./fixture/rd_simu_power_3_weight.Rdata")
```


```{r}
load("./fixture/rd_simu_power_3_weight.Rdata")
ans %>% 
  group_by(`calculated under`) %>% 
  summarize(`simulated power - ss` = mean(ss), 
         `simulated power - invar H0` = mean(invar_H0), 
         `simulated power - invar H1` = mean(invar_H1)) %>% 
  gt::gt() %>% 
