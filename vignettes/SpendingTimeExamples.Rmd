...
target_events <- (xx$analysis %>% subset(hypothesis == "H1"))$Events
...
yy %>% 
  # since we want the power, so we filter the upper bounds
  filter(Bound == "Upper", hypothesis == "H1") %>% 
  mutate(
    # calculate the sample size, which is the same as the original design
    N = sum(xx$enrollRates$duration * xx$enrollRates$rate),
    # calculate the nominal p-value
    `Nominal p` = pnorm(-Z)
  ) %>% 
  select(Time, N, Events, `Nominal p`, AHR, Probability) %>% 
  gt() %>% 
  fmt_number(columns = c("N", "Events"), decimals = 1) %>% 
  fmt_number(columns = c("Nominal p", "AHR", "Probability"), decimals = 4)
```
...
summary_bound(
  xx,
  analysis_vars = c("Time", "N", "Events", "IF", "AHR"),
  analysis_decimals = c(1, 1, 1, 4, 4)) %>% 
...
yy %>% 
  ...
  mutate(
    ...
    N = sum(xx$enrollRates$rate * xx$enrollRates$duration),
    # the spending time is calendar fraction
    spending_time = upar_design_CF$timing,
    # calculate the information fraction
    IF = info0 / max(yy$info0),
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt()
...
FHO %>% 
  filter(Bound == "Upper", Z < Inf, hypothesis == "H1") %>% 
  mutate(
    # calculate the sample size
    N = sum(xx$enrollRates$rate * xx$enrollRates$duration),
    # the spending time is calendar fraction
    spending_time = upar_FHO$timing[3:5],
    # calculate the information fraction
    IF = info0 / max(FHO$info0),
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt()
...
```{r}
FHO %>% 
  filter(Bound == "Lower", abs(Z) < Inf, hypothesis == "H1") %>%
  transmute(
    Time = Time, 
    Events = ceiling(Events),
    "Nominal p" = pnorm(Z), 
    "Information fraction" = info0 / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    "~HR at bound" = exp(-Z / sqrt(info)),
...
max_info0 <- max((xx$analysis %>% subset(hypothesis == "H0"))$info)
upar_actual_IF <- upar_design_IF
...
yy <- gs_power_ahr(
  ...
  # Planned time will drive timing since information accrues faster
  events = 1:2, 
  analysisTimes = (xx$analysis %>% subset(hypothesis == "H1"))$Time,
  upper = gs_spending_bound, 
  upar = upar_actual_IF, 
  lower = gs_b,
  lpar = lpar
...
yy %>% 
  ...
  mutate(
    # calculate the sample size
    N = sum(xx$enrollRates$rate * xx$enrollRates$duration),
    # the spending time is actual IF: info0 from yy / info0 from xx
    spending_time = (yy %>% subset(hypothesis == "H0"))$info / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    # calculate the information fraction
    IF = info0 / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt() %>% 
  fmt_number(columns = c("Events", "N"), decimals = 0) %>%
  fmt_number(columns = "Time", decimals = 1) %>%
  fmt_number(columns = c("~HR at bound", "Probability", "IF"), decimals = 3) %>%
  fmt_number(columns = "Nominal p", decimals = 4)
```
...
yz <- gs_power_ahr(
  enrollRates = xx$enrollRates, 
  failRates = failRates, 
  events = (xx$analysis %>% subset(hypothesis == "H1"))$Events[1:2],
  analysisTimes = (xx$analysis %>% subset(hypothesis == "H1"))$Time,
  upper = gs_spending_bound, 
...
yz %>% 
  ...
  mutate(
    ...
    N = sum(xx$enrollRates$rate * xx$enrollRates$duration),
    # the spending time is actual IF: info0 from yy / info0 from xx
    # spending time is less since we use planned IF to compute spending
    spending_time = planned_IF,
    # calculate the information fraction
    # Note that max_info0 = max((xx$analysis %>% subset(hypothesis == "H0"))$info 
    # is denominator to compute "Information fraction" in table
    IF = info0 / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  ...
  fmt_number(columns = "Time", decimals = 1) %>%
  fmt_number(columns = c("~HR at bound", "Probability", "IF"), decimals = 3) %>%
  fmt_number(columns = c("Nominal p", "AHR"), decimals = 4)
```
...
yy <- gs_power_ahr(
  enrollRates = xx$enrollRates, 
  failRates = failRates, 
  events = c(1, max((xx$analysis %>% subset(hypothesis == "H1"))$Events)),
  analysisTimes = (xx$analysis %>% subset(hypothesis == "H1"))$Time,
  upper = gs_spending_bound, 
  upar = upar_planned_IF, 
  lower = gs_b,
...
yy %>% 
  ...
  mutate(
    N = sum(xx$enrollRates$rate * xx$enrollRates$duration),
    IF = info0 / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    spending_time = (xx$analysis %>% subset(hypothesis == "H0"))$info / max((xx$analysis %>% subset(hypothesis == "H0"))$info),
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt() %>% 
...
summary_bound(positive) %>% 
  filter(Bound == "Efficacy") %>% 
  # calculate the spending time, which is info0 / max(info0)
  mutate(
    `Spending Time` = (positive$analysis %>% filter(hypothesis == "H0"))$info /
                      max((positive$analysis %>% filter(hypothesis == "H0"))$info)) %>% 
  gt() %>% 
  fmt_number(columns = 3:7, decimals = 4)
```
...
overall_planned_bounds <- gs_power_ahr(
  ...
  analysisTimes = (positive$analysis %>% filter(hypothesis == "H1"))$Time,
...
overall_planned_bounds %>% 
  ...
  mutate(
    # calculate the sample size
    # the enrollment rate is an update of the positive design with an inflation factor
    N = ceiling(sum(planned_enrollRates$rate * planned_enrollRates$duration)),
    # calculate the information fraction
    IF = (overall_planned_bounds %>% filter(hypothesis == "H0", Bound == "Upper"))$info0 /
         max((overall_planned_bounds %>% filter(hypothesis == "H0", Bound == "Upper"))$info0),
    # calculate the spending time
    spending_time = planned_IF,
    # calculate the nominal p and approximated hazard ratio
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt() %>% 
  fmt_number(columns = c("Events"), decimals = 0) %>%
  fmt_number(columns = "Time", decimals = 1) %>%
  fmt_number(columns = c("~HR at bound", "Probability", "IF", "Nominal p", "AHR", "spending_time"), decimals = 3) 
```
...
positive_60_enrollRates$duration <- max((positive$analysis %>% filter(hypothesis == "H1"))$N) /
...
positive_60_power <- gs_power_ahr(
  ...
  failRates = failRates %>% filter(Stratum == "Positive"),
  # number of events
  events = (positive$analysis %>% filter(hypothesis == "H1"))$Events, # positive$bounds$Events[1:K],
...
positive_60_power %>% 
  ...
  mutate(
    # calculate the sample size
    # the enrollment rate is an update of the positive design with an inflation factor
    N = ceiling((positive_60_enrollRates %>% filter(Stratum == "Positive"))$duration *
                (positive_60_enrollRates %>% filter(Stratum == "Positive"))$rate),
    # calculate the information fraction
    IF = info0 /
         max((positive$analysis %>% filter(hypothesis == "H0"))$info),
    # calculate the spending time
  ...
  fmt_number(columns = c("Events"), decimals = 0) %>%
  fmt_number(columns = "Time", decimals = 1) %>%
  fmt_number(columns = c("~HR at bound", "Probability", "IF", "AHR", "spending_time"), decimals = 3) %>% 
  fmt_number(columns = "Nominal p", decimals = 4) 
```
...
gs_power_ahr(
  ...
  events = 1:2,
  analysisTimes = (positive_60_power %>% filter(Bound == "Upper", hypothesis == "H1"))$Time,
  ...
  mutate(
    ...
    # calculate the information fraction
    IF = info0 /
         max((overall_planned_bounds %>% filter(hypothesis == "H0"))$info0),
    # calculate the spending time
    spending_time = c(0.8, 1),
    # calculate the nominal p and approximated hazard ratio
    `Nominal p` = pnorm(-Z),
    `~HR at bound` = exp(-Z / sqrt(info))) %>% 
  select(Time, N, Events, IF, spending_time, `Nominal p`, AHR, `~HR at bound`, Probability) %>% 
  gt() %>% 
  fmt_number(columns = c("Events"), decimals = 1) %>%
  fmt_number(columns = "Time", decimals = 1) %>%
  fmt_number(columns = c("~HR at bound", "Probability", "IF", "AHR", "spending_time"), decimals = 3) %>% 
  fmt_number(columns = "Nominal p", decimals = 4) 
```
...
positive_40_enrollRates$duration <- max((positive$analysis %>% filter(hypothesis == "H1"))$N) / 
...
upar_actual_IF$max_info <- max((positive$analysis %>% filter(hypothesis == "H0"))$info)
...
positive_40_power <- gs_power_ahr(
  ...
  events = (positive$analysis %>% filter(hypothesis == "H1"))$Events,
  analysisTimes = NULL,
  # set upper bound
  upper = gs_spending_bound,
  upar = upar_actual_IF, 
...
positive_40_power %>% 
  ...
  mutate(
    # calculate the sample size
    # the enrollment rate is an update of the positive design with an inflation factor
    N = ceiling((positive_40_enrollRates %>% filter(Stratum == "Positive"))$duration *
                (positive_40_enrollRates %>% filter(Stratum == "Positive"))$rate),
    # calculate the information fraction
    IF = info0 /
          max((positive$analysis %>% filter(hypothesis == "H0"))$info),
    # calculate the spending time
...
gs_power_ahr(
  ...
  failRates = failRates,
  events = 1:2,
  analysisTimes = (positive_40_power %>% filter(hypothesis == "H1", Bound == "Upper"))$Time,
  upper = gs_spending_bound,
  upar = upar_overall_planned_IF,
  lower = gs_b,
  lpar = rep(-Inf,2)) %>% 
  ...
  mutate(
    # calculate the sample size
    # the enrollment rate is an update of the positive design with an inflation factor
    N = ceiling(sum(positive_60_enrollRates$duration * positive_60_enrollRates$rate)),
    # calculate the information fraction
    IF = info0 /
          max((overall_planned_bounds %>% filter(hypothesis == "H0"))$info0),
    # calculate the spending time
