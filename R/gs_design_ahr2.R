gs_design_ahr2 <- function(
  ...
  allout <- gs_design_npe2(
    ...
    select(
      c("Analysis", "Bound", "Time", "N", "Events", 
        ...
        "info", "info0", "IF", "hypothesis",
        "~HR at bound", "HR generic (H0)", "HR generic (H1)", "Nominal p" )
  ...
  # --------------------------------------------- #
  bounds <- allout %>% 
    select(all_of(c("Analysis", "Bound", "Probability", "hypothesis",
                    "~HR at bound", "HR generic (H0)", "HR generic (H1)", "Nominal p")))
  # --------------------------------------------- #
  #     get analysis summary to output            #
  # --------------------------------------------- #
  analysis <- allout %>% 
    select(Analysis, Time, N, Events, Z, AHR, theta, info, info0, IF, hypothesis) %>% 
    unique()
