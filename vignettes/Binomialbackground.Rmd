...
```{r}
# set a tibble to store the simulation results
simu_power <- tibble::tibble(INVAR_H0 = as.numeric(), INVAR_H1 = as.numeric(), SS = as.numeric())

# total sample size
N <- 497

# sample size per stratum
N_S1 <- N_total * 1 / sum(1:3) 
N_S2 <- N_total * 2 / sum(1:3) 
N_S3 <- N_total * 3 / sum(1:3) 

# set the method 
weight_method <- "invar_H1"

# begin the simulations
for (simu in 1:4) {
  # observations of 3 strata in FA
  x_e_S1 <- rbinom(n = ceiling(N_S1/2), size = 1, prob = 0.25)
  x_e_S2 <- rbinom(n = ceiling(N_S2/2), size = 1, prob = 0.3)
  x_e_S3 <- rbinom(n = ceiling(N_S3/2), size = 1, prob = 0.5)
  x_c_S1 <- rbinom(n = ceiling(N_S1/2), size = 1, prob = 0.4)
  x_c_S2 <- rbinom(n = ceiling(N_S2/2), size = 1, prob = 0.5)
  x_c_S3 <- rbinom(n = ceiling(N_S3/2), size = 1, prob = 0.6)
  ...
  x <- tibble::tibble(
    ...
    n_c = c(n_c_S1, n_e_S2, n_e_S3),
    p_e = c(p_e_S1, p_e_S2, p_e_S3),
    p_c = c(p_c_S1, p_c_S2, p_c_S3)
  )

  # calculate the statistical information per stratum
  x <- x %>% 
  ...
  ans_new <- tibble::tibble(
    simu_ID = simu,
    method = weight_method,
    power = ans %>% filter(Bound == "Upper") %>% select(Probability) %>% unlist() %>% as.numeric()
  )
  
  simu_power <- rbind(simu_power, ans_new)
}
```
