...
Note that when we have equal randomization, this has the perhaps more recognizable $\hat\sigma^2_0 = \bar p (1-\bar p)\frac{4}{N}.$
...
Now we extend the compuation of $\hat\sigma^2_0$ to $\theta_0\neq 0$ using the restriction that rate estimates $\bar p_{C0}, \bar p_{E0}$ under the null hypothesis satisfy $\bar p_{C0}=\bar p_{E0}+d\theta_0$.
Once solved, we will have
...
Squaring this and applying a continuity correction is a typical two-sided testing approach. Using the normally distributed $Z$ enables one-sided testing. 
The continuity correction is unneeded as shown below.
