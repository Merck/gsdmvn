...
# Set the enrollment table of totally 18 month  
enroll16 <- tibble(
  Stratum = rep("All", 4),     # un-stratified
...
# Adjust enrollment rates to enroll 100 subjects
enroll16$rate <- enroll16$rate * 100 / sum(enroll16$duration * enroll16$rate)

# Put these in a single tibble by scenario
# We will use 16 month enrollment for delayed effect and crossing hazards scenarios
enrollRates <- rbind(
  enroll24 %>% mutate(Scenario = "PH"),
...
The survival curves for these 3 scenarios are shown below:
...
s <- tibble(
  ...
  rate = log(2) / c(rep(16, 4), rep(14, 4), rep(14, 4)),
  hr = c(rep(.7, 4),             # constant hazard ratio of 0.7
         c(1, 1, 1, .575),       # hazard ratio of 1 for 6 months followed by a hazard ratio of 0.575.
         c(1.5, 1.5, .5, .5)     # hazard ratio of 1.5 for 4 months followed by a hazard ratio of 0.5.
         ))
...
The average hazard ratio for these 3 scenarios are shown below:

```{r}
# Du rations to be used in common for all failure rate scenarios
dur <- c(4, 2, 100)
...
hr <- do.call(
  rbind,
  lapply(
    c("PH", "Delayed effect 1", "Delayed effect 2", "Crossing"), 
    function(x){
      gsDesign2::AHR(
...
The number of events for these 3 scenarios are shown below:
...
ss_ahr_fixed <- do.call(
  rbind,
  lapply(c("PH", "Delayed effect 1","Delayed effect 2", "Crossing"), 
         function(x) {
  ...
  lapply(
    ...
    function(x){
      gsdmvn::gs_design_ahr(
        ...
        subset(hypothesis == "H1") %>% 
        mutate(Scenario = "Delayed effect 1")
    }
    )
  )%>% 
  select(Time, N, Events, AHR, Scenario) %>% 
...
For the following fixed design, we use a Max Combo test $Z_{1,1}: FH(0, 0)$, $Z_{2,1}: FH(0, 0.5)$, $Z_{3,1}: FH(0.5, 0.5)$, where $Z_{i,1}$ is the Z-statistics under the $i$-th test and first analysis. 
```{r}
# set the test type of the max combo 
MC_test <- data.frame(
  rho = c(0, 0, .5), 
  gamma = c(0, .5, .5), 
  tau = -1,
...
Under the above Max Combo test, the fixed design under four scenarios are summaried as follows. 
```{r}
do.call(
  rbind,
  lapply(
    ...
    function(x){
      gs_design_combo(
        enrollRates %>% filter(Scenario == x),
        failRates %>% filter(Scenario == x),
        MC_test,
        alpha = 0.025,
        beta = 0.1,
        ratio = 1,
