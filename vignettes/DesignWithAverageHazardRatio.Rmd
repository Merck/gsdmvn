...
We consider design under non-proportional hazards for 
@JTBook
...
library(gsDesign)
library(gsDesign2)
devtools::load_all() # ?? what is this for?
library(ggplot2)
library(dplyr)
library(gt)
library(tidyr)
...
s <- tibble(
  Scenario = c(rep("PH", 4), rep("Delayed effect 1", 4), rep("Delayed effect 2", 4), rep("Crossing", 4)),
  Treatment = rep("Control", 16),
  Month = rep(Month, 4),
  duration = rep(duration, 4),
  rate = log(2) / c(rep(14, 4), rep(11, 12)),
  hr = c(rep(.7, 4),         # constant hazard ratio of 0.7
         ...
         1.5, 1.5, .6, .6)   # hazard ratio of 1.5 for 4 months followed by a hazard ratio of 0.5.
         )
...
ggplot(s, aes(x = Month, y = Survival, col = Scenario, lty = Treatment, shape = Treatment)) + 
  ...
  scale_y_log10(breaks = (1:10) /10, lim = c(.05, 1)) +
...
We note that under the *Delayed effect 1* scenario, the average hazard ratio approaches that of the *PH* scenario after a study duration of about 36 months.
...
Under the 3 NPH scenarios events accumulate faster than under the PH scenario both due to a lower control median and a delayed effect. 
...
For the *PH* and *Delayed effect 1* scenarios we need a similar AHR, number of events and sample size for a 36 month study.
