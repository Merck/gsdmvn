...
test_that("Testing gridpts() vs gridpts_(); easy case",{
  ...
  ref <- gsdmvn:::gridpts_(r = 18, mu = 0, a = -Inf, b = Inf)
...
test_that("Testing gridpts() vs gridpts_(); extreme case 1",{
  ...
  ref <- gsdmvn:::gridpts_(r = 18, mu = 6, a = -2, b = 0)
  expect_equal(new$w, ref$w, tolerance = tolerance)
  expect_equal(new$z, ref$z, tolerance = tolerance)
})
...
test_that("Testing h1() vs h1_()",{
  ...
  expect_equal(
    ...
    abs(gsdmvn:::h1_(r = 18, theta = 2, I = 1, a = -Inf, b= -.5) %>% summarize(plower1 = sum(h)) %>% as.numeric() - x$lower$prob[1]), 
    tolerance = tolerance
  )
  expect_equal(
    abs(sum(h1(r = 18, theta = 2, I = 1, a = 1, b = Inf)$h) - x$upper$prob[1]),
    abs(gsdmvn:::h1_(r = 18, theta = 2, I = 1, a = 1, b = Inf) %>% summarize(plower1 = sum(h)) %>% as.numeric() - x$upper$prob[1]),
    tolerance = tolerance
  )
})
...
test_that("Testing h1() vs h1_() for extreme case produces near 0 probability",{
  expect_equal(sum(h1(r = 18, theta = -30, I = 1, a = -2, b = Inf)$h),
               gsdmvn:::h1_(r = 18, theta = -30, I = 1, a = -2, b = Inf) %>% summarize(pupper = sum(h)) %>% as.numeric(),
               tolerance = tolerance)
})
...
test_that("Testing hupdate() vs hupdate_()", {
  ...
  expect_equal(abs(x$lower$prob[2] -
               ...
               abs(x$lower$prob[2] -
                  gsdmvn:::hupdate_(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1, gm1 = gsdmvn:::h1_(theta = 2, I = 1, a = -.5, b = 3)) %>%
  ...
  expect_equal(abs(x$upper$prob[2] -
               ...
               abs(x$upper$prob[2] -
                  ...
                  gsdmvn:::hupdate_(theta = 2, I = 2, a = 2, thetam1 = 2, Im1 = 1, gm1 = gsdmvn:::h1_(theta = 2, I = 1, a = -.5, b = 3)) %>%
                  summarize(plower2 = sum(h)) %>% as.numeric()),
               tolerance = tolerance
  )
})
...
test_that("Testing hupdate() vs hupdate_() for very extreme tail cases",{
  expect_equal(sum(hupdate(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1,
               ...
               gsdmvn:::hupdate_(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                       gm1 = gsdmvn:::h1_(theta = -30, I = 1, a = -.5, b = 3)) %>%
              summarize(plower2 = sum(h)) %>% as.numeric(),
              tolerance = tolerance)
  expect_equal(sum(hupdate(theta = 30, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                               gm1 = h1(theta = 2, I = 1, a = -.5, b = 3))$h),
               gsdmvn:::hupdate_(theta = 30, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                       gm1 = gsdmvn:::h1_(theta = 2, I = 1, a = -.5, b = 3)) %>%
              summarize(plower2 = sum(h)) %>% as.numeric(),
              tolerance = tolerance)
})
...
test_that("Testing hupdate() vs hupdate_() for somewhat extreme case with non-zero probability",{
  ...
  expect_equal(sum(hupdate(theta = -3, I = 2, a = 0, b = Inf,
               ...
               gsdmvn:::hupdate_(theta = -3, I = 2, a = 0, b = Inf,
                       thetam1 = -3, Im1 = 1,
                       gm1 = gsdmvn:::h1_(theta = -3, I = 1, a = -Inf, b = Inf)) %>%
                 summarize(plower2 = sum(h)) %>% as.numeric(),
               tolerance = tolerance)
  ...
  expect_equal(sum(hupdate(r = 18, theta = -3, I = 2, a = -1, b = Inf,
                               thetam1 = -3, Im1 = 1,
                               gm1 = h1(theta = -3, I = 1, a = -Inf, b = Inf))$h),
               gsdmvn:::hupdate_(theta = -3, I = 2, a = -1, b = Inf,
                       thetam1 = -3, Im1 = 1,
                       gm1 = gsdmvn:::h1_(theta = -3, I = 1, a = -Inf, b = Inf)) %>%
                 summarize(plower2 = sum(h)) %>% as.numeric(),
               tolerance = tolerance)
