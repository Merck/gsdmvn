---
title: "Tutorial to Summarize Bounds Tables in gsdmvn"
output:
  html_document:
    toc: true
    toc_depth: 2
...
vignette: |
  %\VignetteIndexEntry{Tutorial to Summarize Bounds Tables in gsdmvn}
...
Then users can use `gsdmvn::gt_formate()` to format the above R table into a gt table. 
...
By using the design parameters above, one can generate an AHR model by `gs_design_ahr2` as
```{r}
x_ahr <- gs_design_ahr(
...
In the future, if we want to develop rtf, then we can use `rtf_format()`.
...
The above table can also be format by `gt_formate()` as
```{r}
summary_bound(x_wlr, method = "WLR") %>% 
   gt_format(method = "WLR")
```
...
One can use `x_combo$bounds`/`x_combo$analysis` to get a simplified bounds/analysis table, which is similar to that by `x_ahr`.
To get a comprehensive bounds summary table, one can use 
...
One can also use `gt_format()` to custom the gt style of the bounds table.
```{r}
summary_bound(x_combo, method = "COMBO") %>% 
