...
median_tte <- 0.75
enrollRates <- tibble(Stratum = "All", duration = enroll_duration, rate = 2 * n_per_arm / enroll_duration)
failRates <- tibble(Stratum = "All", duration = 1, failRate = log(2) / median_tte, hr = .6, dropoutRate = 0)
x <- gs_info_ahr(
...
gs_power_npe(
    ...
    upar = list(sf = gsDesign::sfLDPocock, total_spend = 0.0125, param = NULL, timing = NULL, max_info = NULL),
    test_upper = TRUE,
    lower = gs_b, 
    lpar = rep(-Inf, 2),
    test_lower = TRUE,
...
# Implementation in Binary Endpoints {.tabset}

Comparison with the old gsdmvn: https://merck.github.io/gsdmvn/articles/PowerEvaluationWithSpending.html. 
...
```{r}
gs_power_npe(theta = h1$theta, theta1 = h1$theta, 
             info = h1$info, info0 = h1$info0, info1 = h1$info1,
             upper = gs_spending_bound,
