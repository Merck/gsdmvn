...
gs_power_ahr(
  ...
  upar = list(sf = gsDesign::sfLDOF, total_spend = 0.025),
  lpar = rep(-Inf, 3), # No futility bound
  events = (design1s$analysis %>% subset(hypothesis == "H1"))$Events[1:3]
) %>%
  filter(Bound == "Upper") %>%
  kable(digits = c(0, 0, 1, 0, 0, 3, 4, 3, 3, 2, 2))
```
...
```{r, message=FALSE}
design2ss$bounds %>% kable(digits = c(0, 0, 4, 0, 4, 4, 4))
```
...
gs_power_nph(
  ...
  filter(abs(Z) < Inf) %>% kable(digits = c(0, 0, 4, 4, 1, 1, 2, 2, 1, 1))
```
