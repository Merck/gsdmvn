...
For testing, we will use $Z=\hat\theta/\hat\sigma_0\sim\hbox{Normal}(0, 1)$
...
We follow the general approach of @LachinBook here.
As shown in the histogram example above, under H$_0$ the proposed statistic $Z\sim \hbox{Normal}(0,1)$.
Letting $\sigma_1^2$ denote the variance under the alternate hypothesis H$_1$ that $Z = \hat\theta/\hat\sigma_0\sim \hbox{Normal}(0,\hbox{Variance} = \sigma^2_0/\sigma^2_1)$.
Thus, there is a role of both $\sigma_0^2$ and $\sigma_1^2$ in power calculations as has been built in to the current package.
The general power calculation under the alternate hypothesis using Type II error notation $1-\beta=$Power:

$$
\begin{aligned}
1-\beta &= P_1\left(Z=(\hat\theta+d\theta_0)/\hat\sigma_0\ge \Phi^{-1}(1-\alpha)\right)\\
 &\approx P_1\left(Z=(\hat\theta+d\theta_1)/\sigma_1\ge \Phi^{-1}(1-\alpha)\right)\\
 &=1 -\Phi\left(\frac{\Phi^{-1}(1-\alpha)\sigma_0+d(\theta_1-\theta_0)}{\sigma_1}\right)

\end{aligned}
$$


Applying the formulas for $\sigma_0$ and $\sigma_1$ makes the final line power calculation a function of $\alpha, N, r, p_E, p_C, d,\theta_0$ which we do not expand further. Inverting that formula gives a sample size formula.
Consistent with other sample size and power calculation problems in the package, we use the null and alternate hypothesis variance formulas and compute statistical information in `gs_info_rd()` and apply this with the standard approach for the package to compute power with `gs_power_rd()` and sample size with `gs_design_rd()`.
The general theory for this is as summarized by @JTBook which will not be repeated here. 

## Stratified population

### Assumptions

We will consider $m>1$ strata with $d, \theta_0, r$ as before, but allow $\theta_{1j}, j=1,\cdots,m$ to vary by stratum.
The full alternate hypothesis is specified by $p_{Ej}, p_{Cj}=p_{Ej}+\theta_{1j}, j=1,\cdots, m$ as well as
positive weights $w_j, j=1,\cdots m$.
Our test is now

$$
\begin{aligned}
Z&=\frac{\sum_{j=1}^m w_j\left(\hat\theta_j + d\theta_0\right)}{\left(\sum_{j=1}^mw_j^2\hat\sigma_{0j}^2\right)^{1/2}}.
\end{aligned}
$$
Overall variance calculations $\sigma_0^2$ and $\sigma_1^2$ are a straightforward extension of the single stratum versions.


### Weighting options

As previously noted, we will consider weighting based on either inverse-variance weights (@MantelHaenszel) or strata sample size weights (Mehrotra2000).
