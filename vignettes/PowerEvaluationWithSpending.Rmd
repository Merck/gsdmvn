...
info_rd <- function(N=200, ratio=1, p1 = .2, p2 = .1, theta = 0){
    ...
    return(tibble::tibble(N = N, N1 = N1, N2 = N2, ratio = ratio, p1 = p1, p2 = p2, p1_tilde = p1t, p2_tilde = p2t, theta = theta, infot = N / sigmat, info = N / sigma, Endpoint = "Risk difference"))
  }
```
...
$$\frac{N_1 p_1+ N_2 p_2}{N_1+N_2}=\frac{N_1 \tilde p_1+ N_2 \tilde p_2}{N_1+N_2}$$
as well as $\tilde p_1 - \tilde p_2 = \theta$

```{r echo=TRUE}
info_rd(theta=c(0,.1,.05)) %>% gt() %>% 
  fmt_number(decimals=0, columns = c("N", "N1", "N2", "ratio")) %>%
  fmt_number(decimals=3, columns = c("p1", "p2", "p1_tilde", "p2_tilde")) %>%
