...
testthat::test_that("Two-sided symmetric design to reproduce gsDesign test.type=2 bounds", {
  ...
  expect_equal(gsd$upper$bound, gsdv$Z[1:K], tolerance = 0.0001)
  expect_equal(gsd$lower$bound, gsdv$Z[(K+1):(2*K)], tolerance = 0.0001)
  expect_equal(gsd$n.I, gsdv$info[1:K], tolerance = 0.5)
