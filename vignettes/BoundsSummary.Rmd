...
Instead of outputting 4 detailed tables (a table of enrollment rates, a table of failure rates, a table of analysis summary, a table of bounds summary), users can get a com pensive summary table by calling `summary_bound(x)`, where `x` is the object returned either by `gs_design_ahr` or `gs_design_wlr`.
...
```{r}
summary_bound(x_design_ahr) %>% 
   gt::gt() %>% 
   gt::fmt_number(columns = c(3:6), decimals = 4) 
```

Please note the `summary_bound()` can also be applied to objected returned by `gs_power_ahr()`. 
For example,
```{r}
summary_bound(x_power_ahr) %>% 
   gt::gt() %>% 
   gt::fmt_number(columns = c(3:6), decimals = 4)
```
...
```{r}
summary_bound(x_design_wlr) %>% 
   gt::gt() %>% 
   gt::fmt_number(columns = c(3:6), decimals = 4)
```

Note that `summary_bound()` can also be applied to summarize an object returned by `gs_power_wlr()`.
```{r}
summary_bound(x_power_wlr) %>% 
   gt::gt() %>% 
   gt::fmt_number(columns = c(3:6), decimals = 4)
```
...
```{r}
summary_bound(x_design_ahr, display_columns = c("Analysis", "Bound", "Z", "Probability")) %>% 
   as_gt()
```
