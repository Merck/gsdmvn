...
test_that("Testing hupdate_() vs known results from gsProbability", {
  ...
  expect_lt(abs(x$lower$prob[2] -
    gsdmvn:::hupdate_(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1, gm1 = h1(theta = 2, I = 1, a = -.5, b = 3)) %>%
      summarize(plower2 = sum(h)) %>% as.numeric()),
    1e-6
  )
  expect_lt(abs(x$upper$prob[2] -
                 # Compare second upper crossing
                 gsdmvn:::hupdate_(theta = 2, I = 2, a = 2, thetam1 = 2, Im1 = 1, gm1 = h1(theta = 2, I = 1, a = -.5, b = 3)) %>%
                 summarize(plower2 = sum(h)) %>% as.numeric()),
            1e-6
  )
})
test_that("Testing hupdate for very extreme tail cases",{
  expect_lt(gsdmvn:::hupdate_(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1, gm1 = h1(theta = -30, I = 1, a = -.5, b = 3)) %>%
                    summarize(plower2 = sum(h)) %>% as.numeric(),
            1e-100)
  expect_lt(gsdmvn:::hupdate_(theta = 30, I = 2, b = 0, thetam1 = 2, Im1 = 1, gm1 = h1(theta = 2, I = 1, a = -.5, b = 3)) %>%
              summarize(plower2 = sum(h)) %>% as.numeric(),
            1e-100)
})
test_that("Testing somewhat extreme case with non-zero probability",{
  ...
  expect_equal(gsdmvn:::hupdate_(theta = -3, I = 2, a = 0, b = Inf,
                       thetam1 = -3, Im1 = 1,
  ...
  expect_equal(gsdmvn:::hupdate_(theta = -3, I = 2, a = -1, b = Inf,
                       thetam1 = -3, Im1 = 1,
                       gm1 = h1(theta = -3, I = 1, a = -Inf, b = Inf)) %>%
                 summarize(plower2 = sum(h)) %>% as.numeric(),
