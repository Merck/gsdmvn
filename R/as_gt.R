...
as_gt <- function(
  ...
  if(method == "wlr" && is.null(footnote)){
    footnote <- list(content = c("approximate weighted hazard ratio to cross bound.", 
                                 "wAHR is the weighted AHR.",
                                 "One-sided p-value for experimental vs control treatment. Values < 0.5 favor experimental, > 0.5 favor control."),
                     location = c("~wHR at bound", NA, "Nominal p"),
                     attr = c("colname", "analysis", "colname"))
  }
  if(method == "combo" && is.null(footnote)){
    footnote <- list(content = c("EF is event fraction. AHR  is under regular weighted log rank test.",
                                 "One-sided p-value for experimental vs control treatment. Values < 0.5 favor experimental, > 0.5 favor control."),
                     location = c(NA, "Nominal p"),
                     attr = "analysis", "colname")
  }
