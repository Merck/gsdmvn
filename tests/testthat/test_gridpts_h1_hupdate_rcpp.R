...
test_that("Testing hupdateRcpp() vs hupdate_()", {
  x <- gsDesign::gsProbability(k=2, n.I=1:2, theta = 2, a= c(-.5,0), b=c(3,2))
  expect_equal(abs(x$lower$prob[2] -
                  sum(hupdateRcpp(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1, gm1 = h1Rcpp(theta = 2, I = 1, a = -.5, b = 3))$h)),
               abs(x$lower$prob[2] -
  ...
  expect_equal(abs(x$upper$prob[2] -
                     sum(hupdateRcpp(theta = 2, I = 2, a = 2, thetam1 = 2, Im1 = 1, gm1 = h1Rcpp(theta = 2, I = 1, a = -.5, b = 3))$h)),
               abs(x$upper$prob[2] -
...
test_that("Testing hupdateRcpp() vs hupdate_() for very extreme tail cases",{
  expect_equal(sum(hupdateRcpp(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                               gm1 = h1Rcpp(theta = -30, I = 1, a = -.5, b = 3))$h),
               hupdate_(theta = 2, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                       gm1 = h1(theta = -30, I = 1, a = -.5, b = 3)) %>%
              summarize(plower2 = sum(h)) %>% as.numeric(),
              tolerance = tolerance)
  expect_equal(sum(hupdateRcpp(theta = 30, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                               gm1 = h1Rcpp(theta = 2, I = 1, a = -.5, b = 3))$h),
               hupdate_(theta = 30, I = 2, b = 0, thetam1 = 2, Im1 = 1,
                       gm1 = h1(theta = 2, I = 1, a = -.5, b = 3)) %>%
              summarize(plower2 = sum(h)) %>% as.numeric(),
              tolerance = tolerance)
})

test_that("Testing hupdateRcpp() vs hupdate_() for somewhat extreme case with non-zero probability",{
  ...
  expect_equal(sum(hupdateRcpp(theta = -3, I = 2, a = 0, b = Inf,
                               thetam1 = -3, Im1 = 1,
                               gm1 = h1Rcpp(theta = -3, I = 1, a = -Inf, b = Inf))$h),
