...
table_gs_design <- function(x, enrollRates, spending_time = NULL, max_info0 = NULL){
  ...
  if (is.null(spending_time)) {
    x$spending_time <- pmin(x$info0 / max_info0, 1)
  }else {
    x$spending_time <- spending_time
  }
  x %>% 
     transmute(
       Time = Time, 
...
# Get upper bounds
#planned_bounds <- xx$bounds %>% filter(Bound == "Upper" & hypothesis == "H1")

# Get the planned number of analyses
#K <- nrow(planned_bounds)

# Get the planned events
#max_events <- max((xx$analysis %>% subset(hypothesis == "H1"))$Events)
...
#planned_time <- (xx$analysis %>% subset(hypothesis == "H1"))$Time
...
# xx$bounds %>% 
#   left_join(xx$analysis) %>% 
#   subset(hypothesis == "H1" & Bound == "Upper") %>% 
#   mutate(spending_time = pmin((xx$analysis %>% subset(hypothesis == "H0"))$info / max_info0, 1)) %>% 
