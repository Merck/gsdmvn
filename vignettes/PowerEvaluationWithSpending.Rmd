...
3)  $\alpha$-spending for efficacy, and
4)  blinded sample size adaptation.
...
+--------+---------------+---------------+---------------------------+
| Par    | Hypothesized  | Test          | $\widehat{\hbox{Var}}(X)$ |
| ameter | $\theta$      |               |                           |
+========+:=============:+:=============:+:=========================:+
| Risk   | $\theta_{r    | \$            | $\h at                    |
| diff   | d} = p_1-p_2$ | X\_{rd}=\hat  | V_{rd}=\frac{1+r}{N}\l ef |
| erence |               | p *1-*\hat    | t(\tilde p_{1,rd}\tilde   |
|        |               | p_2 -         | q_{1,rd}+\tilde p_{2,rd}  |
|        |               | \theta{rd}\$  | \tilde q_{2,rd}/r\right)$ |
+--------+---------------+---------------+---------------------------+
| Risk   | $\theta_{r    | $X_{r r}      | $\hat V_{rr}=\fr ac{      |
| ratio  | r} = p_2/p_1$ | =\theta_{rr } | 1+r}{N}\left(\theta_{r r} |
|        |               | \times\hat p  | ^2\tilde p_{1,rr}\tilde   |
|        |               | _1 -\hat p_2$ | q_{1,rr}+\tilde p_{2,rr}  |
|        |               |               | \tilde q_{2,rr}/r\right)$ |
+--------+---------------+---------------+---------------------------+
